...
namespace Bulwark {
    public class BlockEntityBehaviorFlag : BlockEntityBehavior {
        ...
        //=======================
            ...
            public bool FlagObstracted { get; protected set; }
            ...
            private long? obstractedCheckRef;
        ...
        public BlockEntityBehaviorFlag(BlockEntity blockEntity) : base(blockEntity) {}
            ...
            public override void Initialize(ICoreAPI api, JsonObject properties) {
                ...
                if (this.Banner != null && this.Api is ICoreClientAPI client) {
                    ...
                    client.Tesselator.TesselateShape(this.Banner.Item, Shape.TryGet(client, "bulwarkatlas:shapes/flag/banner.json"), out MeshData meshData);
            ...
            {
                this.obstractedCheckRef = api.Event.RegisterGameTickListener(this.CheckObstructed, 20000);
            }
        ...
        public override void OnBlockBroken(IPlayer byPlayer = null) {
            ...
            if (this.obstractedCheckRef.HasValue) this.Api.Event.UnregisterGameTickListener(this.obstractedCheckRef.Value);
        ...
        //===============================
                                ...
                                if (itemSlot.Itemstack is ItemStack itemStack)
                                    if (itemStack.Collectible?.NutritionProps is FoodNutritionProperties foodNutrition)
                                        this.cellarExpectancy += foodNutrition.Satiety
                                            * itemStack.StackSize
                                            * (BulwarkModSystem.config.ClaimDurationPerSatiety * (1f + this.BlockBehavior.ExpectancyBonus));
            ...
            private void UpdateCellar(float deltaTime) {
                if (this.Stronghold.IsClaimed) {
                    ...
                    float nowDurationPerSatiety = BulwarkModSystem.config.ClaimDurationPerSatiety * (1f + this.BlockBehavior.ExpectancyBonus);
        ...
        private void CheckObstructed(float deltaTime)
        {
            var type = this.Block.Variant["type"];
            var radius = "type" == "city" ? 5 : 3;
            var stickHeight = "type" == "city" ? 4 : 3;
            radius = 1;
            for(int dx = -radius; dx < radius; dx++)
            {
                for(int dz = -radius; dz < radius; dz++)
                {
                    //new BlockPos(dx, 1, dz)
                    var bl = this.Api.World.BlockAccessor.GetRainMapHeightAt(new BlockPos(this.Pos.X + dx, 1, this.Pos.Z + dz));
                    if(this.Pos.Y + stickHeight < bl)
                    {
                        string message = Lang.Get((this.Stronghold.Name is not null ? "{0}" : "One of your claim") + " flag was obstracted and turned off!", this.Stronghold.Name);

                        if (this.Stronghold.GroupUID is int groupUID) (this.Api as ICoreServerAPI).SendMessageToGroup(groupUID, message, EnumChatType.Notification);
                        else (this.Api as ICoreServerAPI).SendMessage((this.Api as ICoreServerAPI).World.PlayerByUid(this.Stronghold.PlayerUID), GlobalConstants.GeneralChatGroup, message, EnumChatType.Notification);
                        this.capturedBy = null;
                        this.Stronghold.PlayerUID = null;
                        this.Api.World.BlockAccessor.MarkBlockDirty(this.Pos);
                        break;
                    }
                    //Block bl = this.Api.World.BlockAccessor.GetBlock(dx, dy, dz);                  
                }
            }
            //this.capturedBy = null;
        }
            ...
            private void Update(float deltaTime) {
                ...
                var cffff = this.TargetPercent - this.CapturedPercent;
                var tp = this.TargetPercent;
            ...
            private void CaptureUpdate(float deltaTime) {
                ...
                if (this.CapturedPercent == 1f) {
                    if (!this.Stronghold.IsClaimed)
                        if (this.Api.World.BlockAccessor.GetTerrainMapheightAt(this.Pos) - this.Pos.Y <= BulwarkModSystem.config.UndergroundClaimLimit) {
